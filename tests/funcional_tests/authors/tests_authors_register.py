...
class AuthorsRegisterTest(AuthorsBaseTest):
    ...
    def test_empty_first_name_error_message(self):
        def callback(form):
            first_name_field = self.get_by_placeholder(form, 'Your firts name')
    ...
    def test_passwords_do_not_match(self):
        def callback(form):
            ...
            form = self.get_form()
            self.assertIn('Senha são diferentes', form.text)
        self.form_field_test_with_callback(callback)
    ...
    def test_user_valid_data_register_successfully(self):
        ...
        self.get_by_placeholder(form, 'Your firts name').send_keys('Firts Name')
        ...
        self.get_by_placeholder(
            form, 'Confirm the password').send_keys('P@ssw0rd1')
